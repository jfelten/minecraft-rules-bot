...
export class SensoryInputs {
  ...
  private botEvents: any = {
    ...
    'whisper':  (username, message, translate, jsonMsg, matches) => { this.brain.postData('whisper', 'event', {username, message, translate, jsonMsg, matches}),
    'actionBar': (jsonMsg); any => { this.brain.postData('actionBar', 'event', {jsonMsg}),
    'message': (jsonMsg); any => { this.brain.postData('message', 'event', {jsonMsg}),
    'login': () => this.brain.postData('login', 'event', {jsonMsg}),
    ...
    'scoreboardDisplayObjective': (scoreboardName: any, position: number) => this.brain.postData('scoreboardDisplayObjective', 'event', {scorboardName, position }),
  ...
  private blockEvents: any = {
    ...
    'chunkColumnLoad': (point: IPoint) =>  (oldBlock: iBlock, newBlock: iBlock) => this.brain.postData('chunkColumnLoad', 'event', {point}),
    'chunkColumnUnload': (point: IPoint) =>  (oldBlock: iBlock, newBlock: iBlock) => this.brain.postData('chunkColumnLoad', 'event', {point}),
    'pistonMove': (block: IBlock, isPulling: boolean, direction: number) => this.brain.postData('pistonMove', 'event', {point}),
  ...
  constructor(mineFlayerApi: mineflayer) {
  ...
  private enableEvents() {
    for (const eventName of Object.keys(this.botEvents)) {
      this.bot.on(eventName, botEvents[eventName]));
    }
